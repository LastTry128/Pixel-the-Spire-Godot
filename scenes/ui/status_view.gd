...
@onready var status_tooltip: VBoxContainer = %StatusTooltips


func _ready() -> void:
	for tooltip: StatusTooltip in status_tooltip.get_children():
		tooltip.queue_free()
...
func show_view(statuses: Array[Status]) -> void:
	for status: Status in statuses:
		...
		status_tooltip.add_child(new_status_tooltip)
